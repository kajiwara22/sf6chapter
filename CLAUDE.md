...
### 高優先度
1. **本番環境での result フィールド反映確認**
   - `ENABLE_R2=true` でテスト実行し、R2 にアップロードされた Parquet の result が実際に反映されているか確認
   - DuckDB で検証

2. **既存動画データの修復**
   - `repair_result_from_intermediate.py` を使用して既存の処理済み動画を修復
   - 複数の動画でテストして信頼性確認
...
- `adr042_validation.ipynb` - ADR-042 バッチ送信vs個別送信の精度比較検証
...
>>>>>>> 4527c24 (docs(adr): Geminiキャラクター認識モデルの 2.5-flash-lite → 3.1-flash-lite (Flex, バッチ送信) への移行（ADR-042）)
