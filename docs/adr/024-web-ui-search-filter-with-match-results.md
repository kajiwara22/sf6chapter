...
1. ADR-024 の承認
2. **UI 実装**: SearchForm.ts、pages.tsx の修正
3. **検索ロジック実装**: search.ts の SQL 生成・バリデーション実装
4. **テスト**: 単キャラ検索、対戦カード検索、エッジケースの動作確認
5. **統合テスト**: Cloudflare Pages デプロイ後の End-to-End テスト
