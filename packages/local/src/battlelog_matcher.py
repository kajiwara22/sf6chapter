...
class CharacterNormalizer:
    ...
    def __init__(self, aliases_file: str | None = None):
        """
        Args:
...
class BattlelogMatcher:
    ...
    def extract_chapter_characters(self, chapter_title: str) -> tuple[str, str] | None:
        """
    ...
    def extract_battlelog_characters(self, replay: dict[str, Any]) -> tuple[str, str]:
        """
        ...
        p1_char = replay.get("player1_info", {}).get("playing_character_tool_name") or replay.get(
            "player1_info", {}
        ).get("character_name", "Unknown")
        p2_char = replay.get("player2_info", {}).get("playing_character_tool_name") or replay.get(
            "player2_info", {}
        ).get("character_name", "Unknown")
        return (p1_char, p2_char)

    def extract_battle_results(self, replay: dict[str, Any]) -> tuple[str, str]:
        """
    ...
    def match_chapter_with_battlelog(
        ...
        logger.debug(f"  チャプター {start_time}s: {chapter_title} → {chapter_chars_set}")

        # ステップ1: 動画公開日時をパース
        try:
            video_time = datetime.fromisoformat(video_published_at.replace("Z", "+00:00"))
        ...
        for replay in battlelog_replays:
            ...
            try:
                replay_time = datetime.fromtimestamp(uploaded_at, tz=UTC)
